...
//CODIGO PARA NUMEROS TECLADO.
$(document).keydown(function(tecla) {
	if (tecla.keyCode > 47 && tecla.keyCode < 58) {
		var tec = tecla.keyCode - 48;
		if (operacion == 0 ) {
			num1 = parseInt(tec);
			pantalla = pantalla + num1;
			$('#pantalla').text(pantalla);
			numero1 = parseInt(pantalla);
			console.log('El numero1 es ' + numero1);
		} else {
			num2 = parseInt(tec);
			pantalla = pantalla + num2;
			$('#pantalla').text(pantalla);
			numero2 = parseInt(pantalla);
			console.log('El numero2 es ' + numero2);
		}
	} else if (tecla.keyCode > 95 && tecla.keyCode < 106) {
		var tec = tecla.keyCode - 96;
		if (operacion == 0 ) {
			num1 = parseInt(tec);
			pantalla = pantalla + num1;
			$('#pantalla').text(pantalla);
